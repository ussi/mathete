...
$M.Utils.setMargin = function(){
	...
	cont.setAttribute("class", "MathetePar");
	cont.setAttribute("className", "MathetePar");

	var inn = document.createElement("span");
	inn.setAttribute("class", "BasePow");
	inn.setAttribute("className", "BasePow");
...
$M.Env.SetUpNumber = function(){
	...
	Number.prototype.getResult = function (ns) {
		var rat = ns.getMode('rational') || false;
		if (rat){
		...
		var round = ns.getMode('float') || $M.DEFAULT_FLOAT_MODE;
		var res = this.toFixed(round) /  1;
		return (Math.abs(res) < $M.getLimitFloor()) ? 0 : res;
	};

	Number.prototype.getHtml = function () {
		...
		//var res = this;
		var baseMantis = this.toString().split("e");
		...
		return $M.Html.block('' + this, "Number");
	};
};
...
$M.Env.SetUpArray = function(){
	...
	Array.prototype.getMatrixHtml = function(alone) {
		...
		for (var i = 0, l = this.length; i < l; i++) {
			row = this[i];
			var tr = document.createElement("tr"), el,
		...
		brLe.setAttribute("class", "maBrLe");
		brLe.setAttribute("className", "maBrLe");
		var brRi = document.createElement("td");
		brRi.setAttribute("class", "maBrRi");
		brRi.setAttribute("className", "maBrRi");
	...
	Array.prototype.getHtml = function () {
		...
		for (var i = 0, l = this.length; i < l; i++) {
			subArr[subArr.length] = this[i].getHtml();
			if (i < l - 1)
...
$M.Html._bracket = function(symb){
	var dv = document.createElement("span");
	dv.setAttribute("class", "Brack");
	dv.setAttribute("className", "Brack");
	dv.innerHTML = symb;
	dv._max_height = true;
	return dv;
};
...
$M.NameSpace = function (nss) {
	...
	this.add = function (name, el) {
		nss[nss.length - 1][name] = el;
	};

	this.get = function (name) {
		for (var i = nss.length - 1; i >= 0; i--) {
			if (typeof nss[i][name] !== "undefined")
				return nss[i][name];
		}
		return undefined;
	};
	...
	this.getMode = function(key){
		return this.modes[key];
	};

	this.setMode = function(key, value){
...
$M.Mode = function(name, value){
	this.isMode = true;
	this.getResult = function(ns){
		ns.setMode(name, value);
		return this;
	};
};
...
$M.Rational.prototype.getHtml = function(alone){
	return this.denom == 1 ? this.nom.getHtml():
				$M.func.get("div", [this.nom, this.denom]).getHtml();
};

$M.Rational.prototype.getResult = function(ns){
	if (ns.getMode('rational')) {
		this._normal();
		if (this.denom == 1) {
			var res = this.nom;
			return res;
...
$M.Article = function (mid, paragraphs, options) {
	this.init(mid, paragraphs, options);
};

$M.Article.prototype.init = function(mid, paragraphs, options){
	$M.Env.setUp();
...
$M.Article.prototype.calc = function () {
	$M.Env.setUp();
	$M.setActiveArticle(this);
	// reset namespace
	this.ns = new $M.NameSpace();
	...
	for (var i = 0, l = this.paragraphs.length; i < l; ++i) {
		...
		if (!el.isPlotPar && el.DOMEl) {
			...
			if (!out || !out.length){
				...
				out.setAttribute("class", "MathetePar MatheteOut");
				out.setAttribute("className", "MathetePar MatheteOut");
				el.DOMEl.appendChild(out);
			} else
				out = out[0];
			out.innerHTML = '';
			out.appendChild(res.getHtml());
		};
	};
	$M.Env.tearDown();
	$M.setActiveArticle(undefined);
...
$M.Paragraph.prototype.getHtml = function () {
	...
	for (var i = 0, l = this.subpars.length; i < l; i++) {
		if (this.subpars[i].isInstruction)
			continue;
		elements[elements.length] = this.subpars[i].getHtml(true);
		if (i < l - 1) {
...
$M.Paragraph.prototype.getResult = function (ns) {
	...
	var res;
	var subpars = [];
	for (var i = 0, l = this.subpars.length; i < l;)
		 subpars[subpars.length] = this.subpars[i++].getResult(ns);
...
$M.func.add('input', {
	...
	getHtml: function (alone) {
		var article = $M.getActiveArticle();
		this.id = "_mathi" + article.id + '-' + article.inputCount;
		article.inputCount++;
		$M.setActiveArticle(article);
		var val = this.args[0];
		val = Number(val);
...
$M.func.add('add', {
	...
	oneResult: function (args, ns) {
		...
		var addEl = function(list, el) {
			...
			// [a , b] + a => [2*a, b]
			// [5*a, b] + a => [6*a, b]
			// [a*b^2, c] + 5*a*b^2 => [6*a*b^2, c]
			var reMul = function(el) {
				var coeff = 1;
				if (el.isNeg) {
					 coeff = -1;
					 el = el.args[0];
				};
				if (el.isMul && el.args[0].isNum){
					coeff = el.args[0];
					el = el.args.length == 2? el.args[1]:$M.func.get('mul', el.args.slice(1));
				};
				return {coeff:coeff, el:el};
			};
			el = reMul(el);
			for (var j = 0, k = list.length; j < k; ++j) {
				elLi = reMul(list[j]);
				if ($M.Utils.equal(el.el, elLi.el)) {
					var newCoeff = ratadd(el.coeff, elLi.coeff);
					if (newCoeff == 0)
						list.pop(j);
					else
						list[j] = $M.func.get('mul', (newCoeff == 1 ? []: [newCoeff]).
											concat(el.el.isMul? el.el.args: el.el));
					return list;
				};
			};
			list[list.length] = el.coeff == 0 ? 0 :
					(el.coeff == 1 ? el.el : $M.func.get('mul',[el.coeff, el.el]));
			return list;
		};

		var syms = new Array(), val = 0, arResult, sInd, ak;
		for (var i = 0, l = args.length; i < l; ++i) {
			arResult = args[i];
			if (arResult.isNum)
				val = ratadd(val, arResult);
			else
				syms = addEl(syms, arResult);
		};
		val = val.getResult(ns);
		if (syms.length > 0) {
			syms.sort(function(a,b){a+='';b+='';a=a.charAt(0)=='-'?a.slice(1):a;b=b.charAt(0)=='-'?b.slice(1):b;return a==b?0:(a>b?1:-1);});
			var el;
			for (var t = 0, k = syms.length; t < k; ++t) {
				el = syms[t];
				if (el.isMul && el.args[0] == '-1'){
					syms[t] = $M.func.get('neg', el.args.length == 2 ?
						el.args[1] : $M.func.get('mul',el.args.slice(1)));
				};
			};

			if (val != 0)
				syms[syms.length] = val;
			var res = syms.length == 1 ? syms[0]: $M.func.get('add', syms);
			return res;
		};
		return val;
	},
	...
	getHtml: function (alone) {
		...
		for (var i = 0, l = this.args.length; i < l; i++) {
			el = this.args[i];
			_h = el.isNeg? el.args[0].getHtml() : el.getHtml();
			_h = (el.isNum && el<0 && i>0)? (-el).getHtml(): _h;
			if (el.isNeg || el.isNum && el<0) {
			var mi = $M.Html.oper("&minus;");
			if (el.isAdd) {
...
$M.func.add('mul', {
	...
	oneResult: function (args, ns) {
		 ...
		 var ratmul = function(f, s){
		 if (ns.getMode('rational')){
			 f = f.getResult(ns);
			 s = s.getResult(ns);
			 var rat = new $M.Rational(f.nom * s.nom,
						   f.denom * s.denom);
			 return rat.getResult(ns);
		 };
		 return f * s;
	 ...
	 var dv = [];
	 for (var i = 0, l = args.length; i < l; i++) {
		 var el = args[i]; //$M.clone(args[i]);
		 if (el == -1){
		 dv[dv.length] = $M.Html.oper("-");
		 continue;
		 }
		 var _h = el.getHtml(true);
		 if (el.isAdd) {
		 dv[dv.length] = $M.Html.brOpen();
		 dv[dv.length] = _h;
		 dv[dv.length] = $M.Html.brClose();
...
$M.func.add('pow', {
	...
	var base = args[0];
	var exp = args[1];
	if (base.isRoot && base.exp.isNum && exp.isNum){
		var coeff;
		if (ns.getMode('rational')){
		var bexp = base.exp.getResult(ns);
		...
		base = nom == 1? base: $M.pow(base.base, nom);
		if (denom == 1)
			return base;
		return denom==2? $M.sqrt(base):$M.root(base, denom);
		};
		coeff = exp/base.exp;
		return coeff==1?base.base:$M.pow(base.base, (exp/base.exp));
	};

	if (ns.getMode('rational') && exp.isNum){
		if (base.isNum){
		if (exp.nom < 0){
			var tmp = base.nom;
			base.nom = base.denom;
		...
		return (new $M.Rational(Math.pow(base.nom, exp.nom), 
			Math.pow(base.denom, exp.nom))).getResult(ns);
		};
		var needDiv = false;
		...
		var res = exp.denom == 1 ? base : $M.func.get("root",
							  [exp.denom, base]);
...
$M.func.add('div', {
	...
	if (nom.isNum && denom.isNum){
		return ns.getMode('rational') ? (new $M.Rational(nom.nom*denom.denom, nom.denom*denom.nom)).getResult(ns): (nom / denom).getResult(ns);
	};
...
$M.func.add('root', {
	...
	var base = args[1];
	var exp = args[0];
	if (base.isPow && base.exp.isNum && exp.isNum){
		if (ns.getMode('rational')){
		var coeff = (new $M.Rational(base.exp.nom *exp.denom, 
					 base.exp.denom*exp.nom)).getResult(ns);
		base = coeff.nom==1? base.base: $M.pow(base.base, coeff.nom);
		return coeff.denom==2? $M.sqrt(base):$M.root(base, coeff.denom);
		};
		return $M.func.get("pow", [base.base, base.exp/exp]).getResult(ns);
	}
	return base.isNum && exp.isNum ? Math.pow(base, 1 / exp).getResult(ns) :
		$M.func.get("root", [base, exp]);
	},
	...
	//baseHtml.style.marginLeft = coeff * 0.23 + "em";
...
$M.func.add('abs', {
	 ...
	 oneResult: function (el, ns) {
		 var va = el[0];
		 return va.isNum ? Math.abs(va) : $M.func.get('abs', [va]);
	 },
	 getHtml: function(alone){
...
$M.func.add('factorial', {
	...
	getHtml: function(alone){
	var baseHtml = this.args[0].getHtml();
	...
	return $M.Html.block([baseHtml, opHtml], "Fact");
	}
});
...
$M.func.add('switch', {
	...
	for (var i = 0, l = args.length; i < l; i++){
		...
		return $M.error('Switch arguments must be array with 2 elements');
		};
		rel = el[0];
		if (rel.isFalse || (rel.isArray && !rel.length) ||
		(rel.isNum && rel == 0)) {
		continue;
		}
		else
...
$M.func.add('_user_fnc', {
	...
	if ($M.recursionCount > $M.REC_DEEP) {
		return $M.error('Syntax', 'To much recursion');
	}

	if (funcInMem.args.length != this.args.length) {
		$M.recursionCount--;
		return $M.error('Syntax', 'Need other quantity of arguments');
	}
	...
	return ns;
	 ...
	 getResult : function (ns) {
		 ...
		 var elData = [];
		 //x = x.getResult(ns);//.getResult(ns);
		 //y = y.getResult(ns);//.getResult(ns);
		 if (xIsSymb){
			 //x = x.getResult(ns);
			 //y = y.getResult(ns);
			 ns.delNS();
		 ...
		 var c = new $M.Plots.Plot2D(this.el,
					 flotData,
					 {width:opt.width, height:opt.height});
			 c.init();
		 c.plot();

		 //$.plot($(this.el), flotData, {legend:{position:opt.legend}});
		 return true;
	 },
	 ...
	 preHtml : function (width, height) {
		 ...
		 div.setAttribute("className", "Plot");
		 return div;
	 },
	 ...
	 getResult : function (ns) {
		 ...
		 if (xIsSymb){
			 ...
			 if (yIsSymb){
				 ...
				 ns.add(yName, y_);
				 //y.getResult(ns);
				 var nZ = z.getResult(ns);//.getResult(ns);
				 if (!nZ.isNum)
					 continue;
		 ...
		 return;
		 var c = new $M.Plots.Plot3D(this.el,
					 flotData,
					 {width:opt.width});
			 c.init();
		 c.plot();
...
// ****************************************
	 ...
	 $M.Plots.Plot2D = function(el, data, options){
	 ...
	 this.height = this.options.height || 200; //this.canvas.height - this.move*2;
	 ...
	 this.init = function(){
		 ...
		 this.xmax = this.xmax; //+ _dltx * 0.05;
		 this.xmin = this.xmin; // - _dltx * 0.05;
		 this.ymax = this.ymax; // + _dlty * 0.05;
		 this.ymin = this.ymin; // - _dlty * 0.05;
		 ...
		 this.yLabels.Class = 'YLabels';
		 //this.yLabels.style.paddingRight = '3px';
		 this.yLabels.style.height = '95%';
		 this.yLabels.style.cssFloat = 'left';
		 this.yLabels.style.styleFloat = 'left';
		 this.yLabels.style.textAlign = 'right';
		 this.yLabels.style.width = widthDelta * .9 + 'px';
		 this.yLabels.style.position = 'relative';

		 this.xLabels = document.createElement('div');
		 this.xLabels.Class = 'XLabels';
		 this.xLabels.style.position = 'relative';
		 this.xLabels.style.height = heightDelta * 0.95 + 'px' ;
		 this.xLabels.style.top = -heightDelta / 4 + 'px' ;
		 this.xLabels.style.marginLeft = widthDelta + 'px' ;
		 this.xLabels.style.clear = 'both';

		 this.canvas = document.createElement('canvas');
		 this.canvas.style.display = 'block';
		 this.canvas.style.cssFloat = 'right';
		 this.canvas.style.styleFloat = 'right';
		 this.canvas.width = this.width - widthDelta;
	 ...
	 this.axis = function(){
		 ...
		 yl.style.fontSize = this.textSize + 'px';
...
// *****************************************
	 ...
	 this.init = function(){
		 ...
		 if (self.xmin >= Number.POSITIVE_INFINITY || self.ymin >= Number.POSITIVE_INFINITY || self.zmin >= Number.POSITIVE_INFINITY)
		 return;
		 ...
		 //[cos(Q/2),sin(Q/2)v]
		 self.quaternion = (new $M.Plots.Quaternion(([0, 1, 0]).mulS(Math.sin(aX)),
								Math.cos(aX))).
	 ...
	 this.newCoord = function(c){
		 ...
		 var nc = {x: c[0] * this.coeffX() - this.deltaX(),
			 ...
			 var newX = xRow[0]*nc.x - xRow[1]*nc.y + xRow[2]*nc.z;// + this.canvas.width/2;
			 var newY = yRow[0]*nc.x - yRow[1]*nc.y + yRow[2]*nc.z;// + this.canvas.height/2;
	 ...
	 this.textPoint = function(point, text){
		 ...
		 _new = true;
		 dv = document.createElement('div');
		 dv.setAttribute("class", "PlotText");
		 dv.setAttribute("className", "PlotText");
